...
function App() {
  ...
  const handleCopySelectedNode = async (node: FileNode, onSuccess: () => void) => {
    if (!treeData) return;
    try {
  ...
  const handleCopySelectedStructure = async (node: FileNode, onSuccess: () => void) => {
    try {
      const rootString = `${node.name}/\n`;
      ...
      <div className="max-w-7xl w-full mx-auto flex flex-col h-full gap-4 md:gap-5 lg:gap-6 xl:gap-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between shrink-0 bg-neutral-900/40 p-4 md:p-5 lg:p-6 rounded-xl border border-neutral-800/60 shadow-sm gap-4 transition-all duration-300">
          <div className="flex flex-col gap-1.5">
            ...
            <p className={`text-[11px] md:text-xs text-neutral-500 font-mono tracking-wide truncate max-w-[260px] md:max-w-md lg:max-w-xl transition-all
                ${treeData ? "px-2.5 py-1 rounded-md bg-neutral-800/60 border border-neutral-700/50 text-neutral-400 inline-block mt-1" : ""}`}>
                {treeData ? treeData.path : "Select a directory to prepare context for AI"}
            </p>
        </div>
          ...
          <div className="flex flex-wrap items-center gap-3 lg:gap-4">
            {treeData && (
              <button
                ...
                >
                  <span className={`w-2 h-2 rounded-full transition-colors duration-200 ${includeFullTree ? "bg-white" : "bg-neutral-600"}`} />
                  Full map
               </button>
            )}
            ...
            {treeData && (
              <button
                ...
                >
                  <RefreshCw size={16} className={isScanning ? "animate-spin text-white" : ""} />
              </button>
            )}
            ...
            >
              {isScanning ? "Scanning..." : treeData ? "Change Folder" : "Select Folder"}
            </button>
            ...
            {treeData && (
              ...
              >
                {isExporting ? (
                  <><Loader2 size={16} className="animate-spin" /> <span className="hidden sm:inline">Bundling...</span></>
                ) : exportSuccess ? (
                  <><CheckCircle2 size={16} /> <span className="hidden sm:inline">Success!</span></>
                ) : (
                  <><Download size={16} /> <span className="hidden sm:inline">Generate .txt</span></>
                )}
              </button>
            )}
          </div>
        ...
        {treeData ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center border border-neutral-800/40 border-dashed rounded-xl bg-neutral-900/10 gap-4 transition-all duration-300 cursor-pointer group"
               onClick={handleSelectFolder}>
                <FolderOpen size={36} className="text-neutral-700 group-hover:text-neutral-500 transition-colors duration-200" />
                <div className="flex flex-col items-center gap-1">
                <p className="text-neutral-400 text-xs lg:text-sm font-medium">Drop a folder, or click to browse</p>
                <p className="text-neutral-600 text-[11px] font-mono">Select a directory to bundle for AI context</p>
                </div>
            </div>
        )}
