...
import { ChevronRight, ChevronDown, Folder, File, FolderOpen, CheckCircle2, Copy, CopyCheck, ListTree, ListChecks } from "lucide-react";
...
export const TreeNode = ({ 
  ...
  const [isSelectedStructureCopied, setIsSelectedStructureCopied] = useState(false);
    ...
    <div className="flex flex-col text-sm">
      <div 
        className={`flex items-center gap-2 py-1.5 px-2 my-0.5 rounded-md group transition-colors duration-100 select-none 
          ${node.is_disabled 
            ...
            : "hover:bg-white/[0.04] active:bg-white/[0.07] cursor-pointer"
          }`}
      ...
      >
        ...
        <input
          ...
          className={`w-4 h-4 rounded-[4px] border-neutral-600 bg-neutral-900/50 text-white accent-neutral-300 transition-all
            ${node.is_disabled ? "cursor-not-allowed opacity-50" : "cursor-pointer"}`}
        />

        {node.is_dir ? (
          isOpen && !node.is_disabled ? <FolderOpen size={16} className="text-neutral-300 ml-1" /> : <Folder size={16} className="text-neutral-500 ml-1" />
        ) : (
          <File size={16} className="text-neutral-600 ml-1" />
        )}
        
        <span className={`truncate font-mono text-[13px] tracking-tight ml-1
        ...
        {node.is_disabled ? (
          ...
          <div className="ml-auto flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
            ...
            {node.is_dir && (
              ...
              >
                {isStructureCopied ? <CheckCircle2 size={15} className="text-emerald-400" /> : <ListTree size={15} />}
              </button>
            )}
            ...
            {node.is_dir && (
              <button
                ...
                className="p-1.5 rounded-md text-neutral-400 hover:bg-neutral-700 hover:text-white"
              >
                {isSelectedStructureCopied ? <CheckCircle2 size={15} className="text-emerald-400" /> : <ListChecks size={15} />}
              </button>
            )}
            ...
            <button
              ...
              title={node.is_dir ? "Copy Selected Content" : "Copy Content (If Selected)"}
              className="p-1.5 rounded-md text-neutral-400 hover:bg-neutral-700 hover:text-white"
            >
              {isSelectedCopied ? <CheckCircle2 size={15} className="text-emerald-400" /> : <CopyCheck size={15} />}
            </button>
            ...
            <button
              ...
              className="p-1.5 rounded-md text-neutral-400 hover:bg-neutral-700 hover:text-white"
            >
              {isCopied ? <CheckCircle2 size={15} className="text-emerald-400" /> : <Copy size={15} />}
            </button>
      ...
      */}
      {node.is_dir && isOpen && node.children && !node.is_disabled && (
        <div className="ml-[22px] pl-3 border-l border-neutral-700/30 mt-0.5">
